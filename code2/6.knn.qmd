...
knn_grid <- grid_space_filling(
  neighbors(range = c(1, 5)),  # 邻居数
  weight_func(values = c("rectangular", "inv", "gaussian", "triangular")),
...
cv_results <- tune_results %>% 
  collect_metrics() %>% 
  filter(neighbors ==5 & .config == 'Preprocessor1_Model18' )
cv_results
write.csv(cv_results,file = 'knn_models/knn_cv_results.csv')
```
...
```{r}
write.csv(ci,file = 'knn_models/Knn_test_results.csv')
```

```{r}
save(list = ls(),file = 'knn_models/knn.Rdata')
```
